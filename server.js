...
app.post('/autorenew/batch', async (req, res) => {
  try {
    ...
    for (const c of customers) {
      ...
      try {
        if (enable) {
          await fetch(`${FS_BASE}/auto-renew-customers/${docId}?key=${FS_KEY}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
...
// POST - batch add VCs only (text paste / excel upload)
app.post('/autorenew/batchvcs', async (req, res) => {
  try {
    const fetch = require('node-fetch');
    const { vcs } = req.body; // array of VC strings
    ...
    console.log(`BatchVCs done: ${success} success, ${failed} failed`);
    res.json({ success: true, processed: success, failed });
  } catch (e) {
