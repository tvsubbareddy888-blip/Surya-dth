...
async function triggerRecharge(vc, amount, orderId) {
  ...
  try {
    ...
    const rawCookies = loginResp.headers.raw()['set-cookie'] || [];
    const cookies1 = rawCookies.map(c => c.split(';')[0]).join('; ');
    ...
    const rawCookies2 = loginPost.headers.raw()['set-cookie'] || [];
    const cookies2 = rawCookies2.map(c => c.split(';')[0]).join('; ');
    const allCookies = cookies1 + '; ' + cookies2;
    console.log('Login status:', loginPost.status);
    
    // Step 3: Get subscriber by VC
    const vcUrl = 'https://phoenix.dishtvbiz.in/LCO/GetLCOSubscribersByVCNo?vcNo=' + vc;
    const vcResp = await fetch(vcUrl, {
      headers: { 'Cookie': allCookies, 'User-Agent': 'Mozilla/5.0', 'X-Requested-With': 'XMLHttpRequest' }
    });
    const vcData = await vcResp.text();
    console.log('VC response status:', vcResp.status);
    console.log('VC data:', vcData.substring(0, 300));
    ...
    return { success: true, loginStatus: loginPost.status, vcStatus: vcResp.status };
  } catch (err) {
